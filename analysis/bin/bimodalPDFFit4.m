function [output] = bimodalPDFFit4(z)
    ...
    % unoccupied sites.
        ...
        [pdf0_c,pdf0_cint] = mle(z1,'pdf',@pdf_2gauss_pos,...
            'start',[mu1_left s1_left -mu1_left*2 s1_left*4 0.9],'alpha',.05);
        warning on
            ...
            pdf0_c=[mu1_left s1_left mu1_left s1_left  0.5];
            pdf0_cint=NaN;
    ...
    output.pdf0 = @(x) pdf_2gauss_pos(x,pdf0_c(1),pdf0_c(2),pdf0_c(3),pdf0_c(4),pdf0_c(5));
    output.pdf0_coeffs = pdf0_c;
    output.pdf0_cints  = pdf0_cint;
    output.pdf0_func = @pdf_2gauss_pos;
...
function y = pdf_2gauss_pos(x,u1,s1,u2,s2,alpha)
    y = alpha*pdf_gauss_pos(x,u1,s1)+(1-alpha)*pdf_gauss_pos(x,u2,s2);
end

function y = pdf_gauss_pos(x,u1,s1)
    y = normpdf(x,u1,s1).*(x>=0)./normcdf(0,u1,s1);
end
