...
%% Process Fit Data
phis=zeros(length(fRs),1);
Ls=zeros(length(fRs),1);
Bs=zeros(length(fRs),1);
for kk=1:length(fRs)
    ci=confint(fRs{kk});
    disp(ci)
...
% Phase
subplot(221);
plot(xvals,phis/pi,'o','linewidth',2,'markerfacecolor',...
...
% Wavelength
subplot(222)
plot(xvals,Ls,'o','linewidth',2,'markerfacecolor',...
    cface1,'markeredgecolor',cedge1)
hold on
ylabel('\lambda (px)');
xlabel(xVar);
...
% Modulation Depth
subplot(224)
plot(xvals,Bs,'o','linewidth',2,'markerfacecolor',...
    cface1,'markeredgecolor',cedge1)
hold on
