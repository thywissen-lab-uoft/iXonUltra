...
% Assign X,Y, and Z
X=1:size(initImg.Z,2);
Y=1:size(initImg.Z,1);
Z=initImg.Z;
...
% Info button
ttstr='Grab current camera settings.';
cdata=imresize(imread(fullfile(mpath,'icons','info.jpg')),[18 18]);
hbAcqInfo=uicontrol(hpAcq,'style','pushbutton','CData',cdata,'callback',@settingsCB,...
...
rbSingle=uicontrol(bgAcq,'Style','radiobutton','String','Single (PWA,BKGD)',...
    'Position',[22 0 200 20],'units','pixels','backgroundcolor','w','Value',1,...
    'UserData','Single');
rbMulti=uicontrol(bgAcq,'Style','radiobutton','String','Multiple (PWA,..,BKGD)',...
    'Position',[22 20 200 20],'units','pixels','backgroundcolor','w',...
    'UserData','Multi');
rbLive=uicontrol(bgAcq,'Style','radiobutton','String','Live',...
    'Position',[22 40 200 20],'units','pixels','backgroundcolor','w',...
    'UserData','Live');
...
tabs(2)=uitab(hpFit,'Title','1','units','pixels');
...
% Make cross hair draggle (MORE OPTIONS TO FOLLOW);
pCrossXDrag=draggable(pCrossX);
pCrossYDrag=draggable(pCrossY);
pCrossXDrag.on_move_callback=@Xupdate;
pCrossYDrag.on_move_callback=@Yupdate;
...
pXF=plot(X,ones(length(X),1),'-','Visible','off','color',co(1,:),'linewidth',2);
...
pYF=plot(X,ones(length(X),1),'-','Visible','off','color',co(1,:),'linewidth',2);
